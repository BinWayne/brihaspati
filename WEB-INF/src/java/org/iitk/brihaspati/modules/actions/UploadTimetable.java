...
import org.iitk.brihaspati.modules.screens.call.timetable.ParseXml;
import org.iitk.brihaspati.modules.screens.call.timetable.Data;

public class UploadTimetable extends SecureAction
{
	...
	public void doUpload(RunData data, Context context) throws Exception
	{
		...
		String path = "/reports/uploads";

		filePath=data.getServletContext().getRealPath(path)+"/"+fileName;
		...
		try {
			...
			xmlFile=data.getServletContext().getRealPath(path)+"/"+st.nextToken()+".xml";
